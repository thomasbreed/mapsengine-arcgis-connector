...
namespace com.google.geo.mapsengine.interop.arcgis.Extension.Tools
{
    public class ToolsCommand_ShareSecureLink : ESRI.ArcGIS.Desktop.AddIns.Button
    {
        // setup and configure log4net
        protected static readonly ILog log = LogManager.GetLogger(typeof(ToolsCommand_ShareSecureLink));

        // keep a reference to the extension for state management
        GoogleMapsEngineToolsExtensionForArcGIS ext;

        public ToolsCommand_ShareSecureLink()
        {
            // initialize and configure log4net, reading from Xml .config file
            log4net.Config.XmlConfigurator.Configure(new System.IO.FileInfo("log4net.config"));

            log.Info("ToolsCommand_ShareSecureLink initializing.");
        ...
        void HandleSelectionChange(object sender, Extension.SelectionChangeEventArgs e)
        {
            ...
            if (e.isSelected)
            {
                ...
                // also, only when there is one feature selected
